...
clear all
capture log close stat_cvdeath
set more off
log using Stat_cvdeath.smcl, name(stat_cvdeath) replace
timer on 1
...
tab indextype cvdeath, row
label var indextype3 "3rd-line Agent"
tab indextype3 cvdeath, row
label var indextype4 "4th-line Agent"
tab indextype4 cvdeath, row
tab indextype5 cvdeath, row
tab indextype6 cvdeath, row
tab indextype7 cvdeath, row
...
foreach var of varlist gender dmdur_cat prx_covvalue_g_i4 prx_covvalue_g_i5 bmi_i_cats hba1c_cats_i2 sbp_i_cats2 ///
	...
	diuretics_all_i {
table indextype `var', contents(n cvdeath mean cvdeath) format(%6.2f) center col
	}

*******************************************************COX PROPORTIONAL HAZARDS REGRESSION*******************************************************
// update censor times for final exposure to second-line agent (indextype)
forval i=0/5 {
	replace cvdeath_exit = exposuretf`i' if indextype==`i' & exposuretf`i'!=.
}

replace cvdeath=0 if cvdeath_exit<death_date

// declare survival analysis - final exposure as last exposure date 
stset cvdeath_exit, fail(cvdeath) id(patid) origin(seconddate) scale(365.25)
...
mi impute chained (regress) bmi_i sbp (mlogit) prx_covvalue_g_i4_clone hba1c_cats_i2_clone = cvdeath `demo2' `comorb2' `meds3' `clin3', add(20)
...
preserve
keep if linked_b==1
egen cvdeath_exit_g = rowmin(tod2 deathdate2 lcd2)
mi stset cvdeath_exit_g, fail(cvdeath) id(patid) origin(seconddate) scale(365.25)
...
preserve 
sts graph, by(indextype) saving(kmplot_cvdeath, replace)  
forvalues i = 1/5{
  tempfile d`i'
  use `d0', clear
  mi extract `i'
...
forval i=0/5 {
 replace cvdeath_exit = exposuret1`i' if indextype==`i' & exposuret1`i'!=.
}
replace cvdeath=0 if cvdeath_exit<death_date
...
set seed 1979
//impute (20 iterations) for each missing value in the registered variables
mi impute chained (regress) bmi_i sbp (mlogit) prx_covvalue_g_i4_clone hba1c_cats_i2_clone = cvdeath `demo2' `comorb2' `meds3' `clin3', add(20) force
...
forval i=0/5 {
 ...
 replace cvdeath_exit = censordate
 drop censor2 censor3 censordate
}

//reset cvdeath to zero patient is censored before the death event
replace cvdeath=0 if cvdeath_exit<death_date
...
set seed 1979
//impute (20 iterations) for each missing value in the registered variables
mi impute chained (regress) bmi_i sbp (mlogit) hba1c_cats_i2 prx_covvalue_g_i4_clone = cvdeath `demo2' `comorb2' `meds3' `clin3', add(20)
...
stset cvdeath_exit, fail(cvdeath) id(patid) origin(seconddate)
