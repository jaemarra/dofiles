...
//keep only subjects in the HES/ONS linked population
keep if linked_b==1
replace cvdeath=0 if cvdeath==.
save cvdeath, replace
...
replace cvdeath=0 if exposure_exit<cvdeath_exit
replace cvdeath_exit=exposure_exit if exposure_exit<cvdeath_exit
...
mi impute chained (regress) bmi_i sbp (mlogit) smokestatus_clone hba1c_cats_i2_clone = cvdeath `demo2' `comorb2' `meds2' `clin2', add(20)
// spit data to integrate time-varying covariates for diabetes meds.
...
save Stat_cvdeath_mi_index, replace
...
set seed 1979
//impute (20 iterations) for each missing value in the registered variables
mi impute chained (regress) bmi_i sbp (mlogit) hba1c_cats_i2_clone smokestatus_clone = cvdeath `demo2' `comorb2' `meds2' `clin2', add(20)
...
save Stat_cvdeath_mi_index3, replace
...
save Stat_cvdeath_mi_any, replace
