...
replace revasc=0 if exposure_exit<revasc_exit
replace revasc_exit=exposure_exit if exposure_exit<revasc_exit
...
mi impute chained (regress) bmi_i sbp (mlogit) smokestatus_clone hba1c_cats_i2_clone = revasc `demo2' `comorb2' `meds2' `clin2', add(20)
// spit data to integrate time-varying covariates for diabetes meds.
...
save Stat_revasc_mi_index, replace
...
set seed 1979
//impute (20 iterations) for each missing value in the registered variables
mi impute chained (regress) bmi_i sbp (mlogit) hba1c_cats_i2_clone smokestatus_clone = revasc `demo2' `comorb2' `meds2' `clin2', add(20)
...
save Stat_revasc_mi_index3, replace
...
save Stat_revasc_mi_any, replace
