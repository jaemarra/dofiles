...
//  task:		Statistical analysis for major cardiovascular events
...
putexcel A1=("Variable") B1=("HR") C1=("SE") D1=("p-value") E1=("LL") F1=("UL") A`x'=("`rowname'") B`x'=(c[`i',1]) C`x'=(c[`i',2]) D`x'=(c[`i',4]) E`x'=(c[`i',5]) F`x'=(c[`i',6])using table2_mace, sheet("Adjusted") modify
}
...
mi impute mvn bmi_i sbp = mace `demo' `comorb' `meds3' `clin2', add(20) by(indextype)
...
graph export kmplot_mace.pdf, replace
...
graph export survplot_mace.pdf, replace

//Testing PH Assumption
stphplot, by(indextype) saving(lnlnplot, replace)
graph export lnlnplot_mace.pdf, replace
...
metan hr ll ul if adj==0 & trt==1, force by(subgroup) nowt nobox nooverall nosubgroup lcols(Subgroup) effect("Hazard Ratio") title(Unadjusted Cox Model MACE Subgroup Analysis for Index Exposure to DPP4i, size(small))
metan hr ll ul if adj==1 & trt==1, force by(subgroup) nowt nobox nooverall nosubgroup lcols(Subgroup) effect("Hazard Ratio") title(Adjusted Cox Model MACE Subgroup Analysis for Index Exposure to DPP4i, size(small))
metan hr ll ul if adj==0 & trt==2, force by(subgroup) nowt nobox nooverall nosubgroup lcols(Subgroup) effect("Hazard Ratio") title(Unadjusted Cox Model MACE Subgroup Analysis for Index Exposure to GLP1RA, size(small))
metan hr ll ul if adj==1 & trt==2, force by(subgroup) nowt nobox nooverall nosubgroup lcols(Subgroup) effect("Hazard Ratio") title(Adjusted Cox Model MACE Subgroup Analysis for Index Exposure to GLP1RA, size(small))
...
local x=`i'+1
local rowname:word `i' of `matrownames'
putexcel A1=("Variable") B1=("HR") C1=("SE") D1=("p-value") E1=("LL") F1=("UL") A`x'=("`rowname'") B`x'=(c[`i',1]) C`x'=(c[`i',2]) D`x'=(c[`i',4]) E`x'=(c[`i',5]) F`x'=(c[`i',6])using table2_mace, sheet("Adjusted2") modify
}
...
forval i=0/5 {
	replace mace_exit = exposuret0`i' if indextype3==`i' & exposuret0`i'!=.
}

// declare survival analysis for single agent exposure to a thirddate
stset mace_exit, fail(mace) id(patid) origin(thirddate) scale(365.35)
...
stptime, title(person-years) per(1000)
putexcel A1= ("Indextype") B1=("Person-Time") C1=("Failures") D1=("Incidence Rate") E1=("Lower Bound") F1=("Upper Bound") G1=("Hazard Ratio") H1=("Lower Bound") I1=("Upper Bound") using table2_mace, sheet("SensAnalysis3") modify
forval i=0/5{
local row=`i'+2
stptime if indextype==`i'
putexcel A`row'= ("`i'") B`row'=(r(ptime)) C`row'=(r(failures)) D`row'=(r(rate)*1000) E`row'=(r(lb)*1000) F`row'=(r(ub)*1000) using table2_mace, sheet("SensAnalysis3") modify
}
forval i=1/5 {
local row=`i'+2
local matrow=`i'+1
...
matrix b=r(table)
matrix a= b'
putexcel G`row'=(a[`matrow',1]) H`row'=(a[`matrow',5]) I`row'=(a[`matrow',6]) using table2_mace, sheet("SensAnalysis3") modify
}
...
local x=`i'+1
local rowname:word `i' of `matrownames'
putexcel A1=("Variable") B1=("HR") C1=("SE") D1=("p-value") E1=("LL") F1=("UL") A`x'=("`rowname'") B`x'=(c[`i',1]) C`x'=(c[`i',2]) D`x'=(c[`i',4]) E`x'=(c[`i',5]) F`x'=(c[`i',6])using table2_mace, sheet("SensAnalysis3") modify
}
...
forval i=0/5 {
	replace mace_exit = exposuretf`i' if indextype4==`i' & exposuretf`i'!=.
}

// declare survival analysis - final exposure as last exposure date 
stset mace_exit, fail(mace) id(patid) origin(fourthdate) scale(365.25)
...
stptime, title(person-years) per(1000)
putexcel A1= ("Indextype") B1=("Person-Time") C1=("Failures") D1=("Incidence Rate") E1=("Lower Bound") F1=("Upper Bound") G1=("Hazard Ratio") H1=("Lower Bound") I1=("Upper Bound") using table2_mace, sheet("Unadjusted4") modify
forval i=0/5{
local row=`i'+2
stptime if indextype4==`i'
putexcel A`row'= ("`i'") B`row'=(r(ptime)) C`row'=(r(failures)) D`row'=(r(rate)*1000) E`row'=(r(lb)*1000) F`row'=(r(ub)*1000) using table2_mace, sheet("Unadjusted4") modify
}
forval i=1/5 {
local row=`i'+2
local matrow=`i'+1
...
matrix b=r(table)
matrix a= b'
putexcel G`row'=(a[`matrow',1]) H`row'=(a[`matrow',5]) I`row'=(a[`matrow',6]) using table2_mace, sheet("Unadjusted4") modify
}
