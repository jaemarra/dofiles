...
forval i=0/49 {
	...
	keep patid gemscriptcode rxdate2 qty ndd studyentrydate_cprd2 issueseq
...
//coding for each antidiabetic drug exposure: 0=no exposure and 1=exposure.
...
//enumerate overall prescription order
...
format first %td
label var first "Earliest date for any antidiabetic prescription"
//Pull out first prescription overall
gen firstadmrx = .
replace firstadmrx=rxtype if first==rxdate2
//Check for greater than 1 type of prescription in first overall variable
...
//GENERATE SWITCH/ADD EXPOSURE VARIABLES 
//Generate start exposure time variables for each rxtype
...
//Generate stop exposure time variables for each rxtype
foreach var of varlist metformin sulfonylurea dpp glp insulin tzd otherantidiab {
bysort patid: egen `var't2= max(rxdate2) if `var'==1
format `var't2 %td	
label var `var't2 "Last `var' exposure date ever"
}
...
//GENERATE EXPOSURE TIME VARIABLES
...
forval i=0/49 {
use Therapy_`i'dm, clear
//GENERATE DURATION FOR "EXPOSED" PERIODS FOR SURVIVAL ANALYSES
//pull out every t0 and the associated t1 by looking for
gen exposure_b=.
...
gen t1=.
gen duration=.
//All prescription dates refer to an exposure start date (t0)
replace t0=rxdate2
format t0 t1 %td
foreach var of varlist metformin sulfonylurea dpp glp insulin tzd otherantidiab {
...
//GENERATE DURATION FOR "UNEXPOSED" PERIODS FOR SURVIVAL ANALYSES 
...
//Generate duration: total follow-up time available minus gaptime
...
foreach var of varlist metformin sulfonylurea dpp glp insulin tzd otherantidiab {
summarize `var'_dur, detail
}
compress
save ExposuresADM_`i'.dta, replace
	}
use ExposuresADM_0, clear 
forval i=1/49 {		
