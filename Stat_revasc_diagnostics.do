//  program:    Stat_revasc_diagnostics.do
...
clear all
capture log close Stat_revasc_diagnostics
set more off
log using Stat_diagnostics.smcl, name(Stat_revasc_diagnostics) replace
timer on 1
...
sort _t
twoway scatter surv2 _t, c(stairstep) ms(i) || scatter surv3 _t, c(stairstep) ms(i) || scatter surv4 _t, c(stairstep) ms(i) || scatter surv5 _t, c(stairstep) ms(i) || scatter surv6 _t, c(stairstep) ms(i) || scatter surv7 _t, c(stairstep) ms(i) ti("Averaged Curves") saving(avgkmplot_revasc, replace)
...
timer off 1
log close Stat_revasc_diagnostics
