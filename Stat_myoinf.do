...
clonevar myoinf=mi
replace myoinf=0 if myoinf==.
clonevar myoinf_exit=mi_exit
save myoinf, replace
...
replace myoinf=0 if exposure_exit<myoinf_exit
replace myoinf_exit=exposure_exit if exposure_exit<myoinf_exit
...
mi impute chained (regress) bmi_i sbp (mlogit) smokestatus_clone hba1c_cats_i2_clone = myoinf `demo2' `comorb2' `meds2' `clin2', add(20)
// spit data to integrate time-varying covariates for diabetes meds.
...
set seed 1979
//impute (20 iterations) for each missing value in the registered variables
mi impute chained (regress) bmi_i sbp (mlogit) hba1c_cats_i2_clone smokestatus_clone = myoinf `demo2' `comorb2' `meds2' `clin2', add(20)
