...
forval i=0/5 {
	replace myoinf_exit = exposuretf`i' if indextype==`i' & exposuretf`i'!=.
}

replace myoinf=0 if myoinf_exit<death_date

// declare survival analysis - final exposure as last exposure date 
stset myoinf_exit, fail(myoinf) id(patid) origin(seconddate) scale(365.25)
...
preserve
keep if linked_b==1
egen myoinf_exit_g = rowmin(tod2 deathdate2 lcd2)
mi stset myoinf_exit_g, fail(myoinf) id(patid) origin(seconddate) scale(365.25)
...
forval i=0/5 {
 replace myoinf_exit = exposuret1`i' if indextype==`i' & exposuret1`i'!=.
}
replace myoinf=0 if myoinf_exit<death_date
...
forval i=0/5 {
 ...
 replace myoinf_exit = censordate
...
stset myoinf_exit, fail(myoinf) id(patid) origin(seconddate)
